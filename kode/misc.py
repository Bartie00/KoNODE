...
class _PerturbFunc(torch.nn.Module):
    ...
    def forward(self, t, y, *, perturb=Perturb.NONE):
        ...
        # print(t,y.dtype)
        # print("self.base_func(t, y)",t,y,self.base_func(t, y))
        #return self.base_func(t, y)
        # print("pertub_func",self.base_func)
        w = self.base_func(t, y)
        # print("w",w.shape)
        return w
...
def _check_inputs(func, y0, t, rtol, atol, method, options, event_fn, SOLVERS):
    ...
    if is_tuple:
        ...
        # y0 = torch.cat([y0_.to_dense().reshape(-1) for y0_ in y0])
    ...
    if is_tuple:
        ...
        def _norm(tensor):
            # print("misc_tensor",tensor.shape,shapes)
            y = _flat_to_shape(tensor, (), shapes)
            # print("misc_norm",norm(y).shape)
            return norm(y)
        options['norm'] = _norm
